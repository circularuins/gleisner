...
- **Skip-when-unchanged assumes the existing DB URL was already validated.** Update mutations (`updatePost`, `updateMyArtist`, `updateMe`) skip the magic-byte check when the submitted URL equals the URL already stored on the row. This is sound for any URL written **after** ADR 026 lands, because every persisted URL was validated at write time. It is **not** sound for URLs written before ADR 026 — those bytes were never magic-byte-checked. Phase 0 (Gleisner) has no production data older than this PR, so the skip path is safe at first deploy. Any future environment that imports pre-ADR-026 data must run a one-time backfill validation pass before relying on the skip (tracked in Issue #278). The skip itself relies on strict-equality string comparison; R2 public URLs are machine-generated by `generateUploadUrl` and are stable in form (no trailing-slash or case variants), so equality is the correct primitive.
