...
5. **旧ウィジェットを完全置換する場合（並存させない場合）は、callsite ゼロを確認後に旧ファイルを `git rm` する**。残すと dead code 化し、その後の i18n / lint 一括修正で live コードと区別できなくなる
6. **旧ファイル削除に伴い、その orphan になる i18n キー・テストフィクスチャ・他ファイル内のコメント参照も同時に削除する**。`grep -rn "旧キー名" lib/ test/` で確認

PR #337 の教訓: PR #57 で `RegisterArtistSheet` → `RegisterArtistWizard` 完全置換時に旧ファイルを削除し忘れ、その後 PR #216/#260/#268 で「live コードと思われて」継続的に磨かれた。約2ヶ月後に外部 FB 起点の clarify 作業中に発見し、PR #337 後半で削除（181 行）。

### 外部画像の Image.network には cacheWidth を指定する

**外部ドメインの画像（OGP サムネイル等）を `Image.network` で表示する場合、`cacheWidth` を必ず指定すること。** 元画像サイズが予測不能なため、フルサイズでメモリにデコードされる。

```dart
// ❌ cacheWidth なし — 4K 画像がそのままメモリ展開
Image.network(ogImageUrl, fit: BoxFit.cover)

// ✅ 表示サイズに合わせて制限
Image.network(ogImageUrl, fit: BoxFit.cover, cacheWidth: 800)
// ノードカード等の小サイズ: cacheWidth = (node.width * 2).toInt()
// 詳細シート等の大サイズ: cacheWidth = 800
```

R2 にアップロードされた自アプリの画像は、アップロード時にリサイズ済みのため `cacheWidth` は不要。
...
### メディアアップロード前のバリデーション順序（R2 orphan 防止）

**R2 にファイルをアップロードする前に、ローカルで検証可能な項目（duration, format, size 等）を全てチェックすること。** アップロード後にバリデーションで弾くと、R2 にファイルが残ったまま孤児になる。

```dart
// ❌ アップロード → バリデーション（orphan 生成）
final url = await _upload(bytes: bytes, ...);
final duration = await extractDuration(bytes);
if (duration > maxDuration) { return null; } // R2 に orphan が残る

// ✅ バリデーション → アップロード（orphan なし）
final duration = await extractDuration(bytes);
if (duration == null) { state = error; return null; } // 不明ならブロック
if (duration > maxDuration) { state = error; return null; }
final url = await _upload(bytes: bytes, ...);
```
...
### bytes を変換するサニタイズは getUploadUrl 呼び出し前に完了させる

**R2 presigned URL は `ContentLength` を署名対象に含むため、サニタイズ・変換・再エンコード等で bytes 長が変わる処理は必ず `getUploadUrl` mutation 呼び出し前に完了させること。** `_upload()` 内部で行うと署名時の `contentLength` と実際の PUT body サイズが不一致になり、R2 が `SignatureDoesNotMatch` で拒否する。

```dart
// ❌ _upload() 内部でサニタイズ — 署名時の長さと PUT body の長さが不一致
Future<String?> _upload({required Uint8List bytes, ...}) async {
  final sanitized = await sanitize(bytes); // ← 長さが変わる
  await _gqlClient.mutate(...);            // ← 元の長さで署名
  await _httpClient.put(body: sanitized);  // ← 新しい長さで送信 → 401
}

// ✅ mutation 呼び出し前に完了させる
final sanitized = await sanitize(bytes);
if (sanitized == null) { state = error; return null; }
return await _upload(bytes: sanitized.bytes, contentType: sanitized.contentType);
```

対象: EXIF 除去、HEIC 変換、画像圧縮、動画サムネイル抽出、その他 byte 長を変える前処理全般。Backend-implementation.md の「R2/S3 presigned URL の注意事項」に対応するフロントエンド側の順序制約。

PR #231 の教訓: EXIF サニタイザの挿入位置をコメントで明記し、将来 `_upload()` 内部に移されるリグレッションを防止した。

### image_picker の `imageQuality` は JPEG のみ再エンコード

**`ImagePicker.pickImage(imageQuality: N)` は JPEG フォーマットにしか再エンコードを適用しない。** iOS は `UIImageJPEGRepresentation`、Android は `Bitmap.compress(JPEG, ...)` を内部で呼ぶため、PNG/WebP/HEIC は実質的に無変換で通過する。以下の目的で `imageQuality` に依存している場合、PNG/WebP は保護されないので注意:

- プライバシー（EXIF/XMP メタデータの除去）
- ファイルサイズ削減（圧縮）
- 正規化（デコード → 再エンコードによる形式統一）

```dart
// ❌ imageQuality だけでは PNG/WebP の EXIF が素通り
final picked = await picker.pickImage(imageQuality: 75);

// ✅ 明示的なサニタイズで全フォーマットをカバー
final picked = await picker.pickImage(imageQuality: (q ?? 75).clamp(1, 85));
final bytes = await picked.readAsBytes();
final sanitized = await sanitizeImageMetadata(bytes, contentType: detected);
```

**クランプも必要**: `imageQuality: 100` や `null` は再エンコードをスキップする実装バージョンがあるため、1-85 にクランプして再エンコードを強制する。

PR #231 の教訓: ネイティブ向けに `imageQuality` 再エンコードに依存していたが、レビューで PNG/WebP 素通りを指摘され、fail-close へ切り替え（Issue #227 で `image` パッケージ経由の明示サニタイズに移行予定）。

### バイナリマーカーは byte array 定数で記述する（git バイナリ誤検出防止）

**EXIF マーカー・magic bytes・プロトコルヘッダ等に NUL (`0x00`) などの制御バイトが含まれる場合、Dart 文字列リテラル（`'Exif\x00\x00'`）で書くと git がファイル全体をバイナリとして認識する。** `.dart` ファイルが `Bin 0 -> N bytes` 扱いになり、PR の diff がレビューできなくなる。

```dart
// ❌ NUL 埋め込みで git がバイナリ判定 → PR レビュー不能
if (ascii.contains('Exif\x00\x00')) return true;

// ✅ byte array 定数で ASCII-clean に保つ
const _exifMarker = <int>[0x45, 0x78, 0x69, 0x66, 0x00, 0x00]; // "Exif\x00\x00"
for (int i = 0; i + _exifMarker.length <= len; i++) {
  var match = true;
  for (int j = 0; j < _exifMarker.length; j++) {
    if (bytes[i + j] != _exifMarker[j]) { match = false; break; }
  }
  if (match) return true;
}
```

git の バイナリ判定は最初の数 KB に NUL が含まれるかで決まるため、コメント内の `\x00` 表記は問題にならない（実体は `\` `x` `0` `0` というテキスト）。問題になるのは文字列リテラルで解釈された結果のバイト列だけ。

PR #231 の教訓: `'Exif\x00\x00'` を書いたことで `image_sanitizer.dart` が git にバイナリ判定され、初回 PR で diff が `Bin 0 -> 6310 bytes` と表示されていた。byte array 定数に書き換えて amend。
