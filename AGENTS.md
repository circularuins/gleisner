...
このファイルは Gleisner プロジェクト固有の **AI コーディングエージェント全般** の指針です。Claude Code / Codex CLI などツールを問わず、Gleisner で作業する際は必ず参照してください。
...
バックエンド・フロントエンドの詳細な実装ルール（UserType 分離、認可チェック、Provider 層ルール等）は `.claude/rules/` 配下に集約されている:
...
> Claude Code は path-scoped で当該パスのファイルを読んだ時にのみ自動 load する（[公式仕様](https://code.claude.com/docs/en/memory#path-specific-rules)）。
